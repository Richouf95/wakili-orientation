...
function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name || !email || !telephone || !pwd) {
      setErrorFront("Veuillez remplir tous les champs.");
      return;
    }

    try {
      await signup(name, email, telephone, pwd);
    } catch (error) {
      console.error(error);
      setErrorFront("Erreur d'inscription. Veuillez réessayer.");
    }
  };
  ...
  return (
    <div className="flex justify-center items-center px-5 mb-10 md:p-10">
      ...
      >
        <h2 className="text-3xl mb-5 mt-2 text-center font-bold">
          Inscription
        </h2>
        <form onSubmit={handleSubmit}>
          <Box sx={{ display: "flex", flexWrap: "wrap", width: 1 }}>
            ...
            {errorFront && (
              <p className="text-red-500 my-1 text-center">{errorFront}</p>
            )}
            {error && <p className="text-red-500 my-1 text-center">{error}</p>}
            <div className="flex justify-center w-full">
              {isLoading ? (
                <button
                  disabled
                  className="my-5 px-10 py-3 w-full md:w-3/5 bg-[#505958c1] btn text-white font-bold rounded-full"
                >
                  <svg
                    aria-hidden="true"
                    role="status"
